                ...
                return (
                    <div key={ev.id} className="flex justify-between items-center bg-white p-2 mb-2 rounded shadow-sm border-l-4" style={{ borderColor: color }}>
                        ...
                        <div className="flex flex-col space-y-1">
                            {ev.isReleasable && (
                                <button onClick={() => handlePPGoalScored(ev.id)} className="px-2 py-1 bg-blue-50 text-blue-600 text-xs font-bold rounded border border-blue-200 hover:bg-blue-100 transition">PPG Scored</button>
                            )}
                            {isMajor && (
                                <button onClick={() => handleMajorPenaltyRelease(ev.id)} className="px-2 py-1 bg-green-50 text-green-700 text-xs font-bold rounded border border-green-300 hover:bg-green-100 transition">Released</button>
                            )}
                            {!isMajor && (
